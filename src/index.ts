 ...
 * @param options.disableDiagnostics - Disables MCPCat's internal SDK diagnostics (anonymous error/telemetry reporting used to monitor SDK setup failures). Diagnostics are on by default and can also be disabled with the `MCPCAT_DISABLE_DIAGNOSTICS` environment variable. Local `~/mcpcat.log` logging is unaffected.
